...
class Department {
    ...
    static async create(client,  { name, comment, organization_id, parent_id }) {
    ...
    static async update(client, id, { name, comment, organization_id, parent_id }) {
        const { rows } = await client.query(
    ...
    //     const { rows } = await client.query(
    //         `UPDATE department
    //          SET name = $1, comment = $2, organization_id = $3, parent_id = $4,
    ...
    static async delete(client, id) {
        await client.query('UPDATE department SET deleted_at = current_timestamp WHERE id = $1 AND deleted_at IS NULL', [id]);
