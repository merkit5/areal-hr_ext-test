...
const passportSchema = Joi.object({
  series: Joi.string().length(4).pattern(/^\d+$/).required(),
  number: Joi.string().length(6).pattern(/^\d+$/).required(),
  issue_date: Joi.date().iso().required(),
  issuer_code: Joi.string()
    .pattern(/^\d{3}-\d{3}$/)
    .required(),
  issuer: Joi.string().min(5).max(255).required(),
});

const addressSchema = Joi.object({
  region: Joi.string().min(2).max(255).required(),
  locality: Joi.string().min(2).max(255).required(),
  street: Joi.string().min(2).max(255).optional().allow(''),
  house: Joi.string().min(1).max(10).optional().allow(''),
  building: Joi.string().min(1).max(10).optional().allow(''),
});

const createEmployeeSchema = Joi.object({
  first_name: Joi.string().min(2).max(255).required(),
  last_name: Joi.string().min(2).max(255).required(),
  patronymic: Joi.string().min(2).max(255).optional().allow(''),
...
const updateEmployeeSchema = Joi.object({
  ...
  patronymic: Joi.string().min(2).max(255).allow(''),
